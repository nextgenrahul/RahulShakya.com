...
import { motion } from "framer-motion";
import { Activity, Database, Sparkles, LayoutGrid } from "lucide-react";

export default function ServicesSpecs() {
  const specs = [
    { title: "Target App Latency", val: "< 50ms", label: "Developer", icon: <LayoutGrid className="w-4 h-4 text-[#0071e3]" /> },
    { title: "Algorithmic Precision", val: "99.4%", label: "Data Scientist", icon: <Database className="w-4 h-4 text-[#0071e3]" /> },
    { title: "Avg Audience Retention", val: "85%+", label: "Video Editor", icon: <Sparkles className="w-4 h-4 text-[#0071e3]" /> },
    { title: "Studio SLA Delivery", val: "On Time", label: "Core Guarantee", icon: <Activity className="w-4 h-4 text-[#0071e3]" /> }
  ];
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {specs.map((spec, i) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              className="bg-white p-6 rounded-2xl border border-[#d2d2d7]/40 shadow-xs space-y-4 relative overflow-hidden"
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono font-bold text-[#86868b] uppercase tracking-wide bg-[#f5f5f7] px-2 py-0.5 rounded-md">
                  {spec.label}
                </span>
                {spec.icon}
              </div>
